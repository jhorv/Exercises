...
type ColorWheel() as t =
    inherit ListBox()

    do
        let factoryRadialPanel = FrameworkElementFactory(typeof<RadialPanel>)
        t.ItemsPanel <- ItemsPanelTemplate(factoryRadialPanel)

        let template = DataTemplate(typeof<Brush>)
        t.ItemTemplate <- template

        let elRectangle = FrameworkElementFactory(typeof<Rectangle>)
        elRectangle.SetValue(Rectangle.WidthProperty,4.0)
        elRectangle.SetValue(Rectangle.HeightProperty,12.0)
        elRectangle.SetValue(Rectangle.MarginProperty,Thickness(1.0,8.0,1.0,8.0))
        elRectangle.SetBinding(Rectangle.FillProperty,Binding "")

        template.VisualTree <- elRectangle
        typeof<Brushes>.GetProperties()
        |> Array.iter(fun prop -> t.Items.Add(prop.GetValue(null)) |> ignore)

[<STAThreadAttribute>]
try 
    let win = Window()
    win.Title <- "Select Color from Wheel"
    ...
    let clrWheel = ColorWheel()
    clrWheel.Margin <- Thickness 24.0
    clrWheel.HorizontalAlignment <- HorizontalAlignment.Center
    clrWheel.VerticalAlignment <- VerticalAlignment.Center
    stack.Children.Add clrWheel |> ignore

    clrWheel.SetBinding(ColorWheel.SelectedValueProperty,"Background") |> ignore
    clrWheel.DataContext <- win
