...
try 
    let win = Window()
    win.Title <- "Enclose Element In Ellipse"

    let elips = EllipseWithChild()
    elips.Fill <- Brushes.ForestGreen
    elips.Stroke <- Pen(Brushes.Magenta,48.0)
    win.Content <- elips

    let txt = TextBlock()
    txt.FontFamily <- FontFamily("Times New Roman")
    txt.FontSize <- 48.0
    txt.Text <- "Text inside ellipse"
