  ...
  main = do
    ...
    s <-
      forM [1..num_test_cases] $ \_ -> do
        [_,m] <- parse <$> B.getLine
        x <- V.fromList . map (\x -> fromIntegral $ x-1) . parse <$> B.getLine :: IO (Vector Word32)
        d <- V.map fromIntegral . V.fromList . parse <$> B.getLine :: IO (Vector Word32)
        let
          xd =
            V.filter (\(x,d) -> d >= 2) (V.zip x d)
            |> V.map (\(x,d) -> (min (d - x - 1) x, d `div` 2 + d `mod` 2))
            |> V.unzip

          in do return $ solve xd (fromIntegral m)
